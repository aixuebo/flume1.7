...
//执行一个工具对象,输入工具name以及对应的参数即可调用该工具
public class FlumeToolsMain implements FlumeTool {
  ...
  public void run(String[] args) throws Exception {
    ...
    String toolName = args[0];//获取工具类型name
    FlumeTool tool = null;//工具类型对应的实例对象
    for (FlumeToolType type : FlumeToolType.values()) {
      if (toolName.equalsIgnoreCase(type.name())) {//找到参数name对应的class
