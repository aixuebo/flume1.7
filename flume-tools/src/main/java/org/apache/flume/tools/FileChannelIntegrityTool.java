...
/**
 -l 表示dataDirs,即输入目录集合,该目录集合用逗号拆分,必须都存在的目录
 -h 表示help
 -e 表示eventValidator,输入事件的校验器实现类
 -Dproperty=value 输入对应的参数
 */
public class FileChannelIntegrityTool implements FlumeTool {
  ...
  private EventValidator eventValidator = EventValidator.NOOP_VALIDATOR;//默认无论怎么校验都返回true
  ...
  public void run(String[] args) throws IOException, ParseException {
    boolean shouldContinue = parseCommandLineOpts(args);//解析参数
    if (!shouldContinue) {//false表示解析失败
      LOG.error("Could not parse command line options. Exiting ...");
      System.exit(1);
    }
    for (File dataDir : dataDirs) {//循环每一个目录
      File[] dataFiles = dataDir.listFiles(new FilenameFilter() {//获取该目录下的所有子文件组成的集合,子文件刨除一部分规则的文件名

          //true表示是要的文件
        @Override
        public boolean accept(File dir, String name) {//该文件本身,以及该文件的子文件name
          if (!name.endsWith(Serialization.METADATA_FILENAME) //.meta
              && !name.endsWith(Serialization.METADATA_TMP_FILENAME) //.tmp
              && !name.endsWith(Serialization.OLD_METADATA_FILENAME) //.old
              && !name.equals(Log.FILE_LOCK)) { //in_use.lock
              //不是以上的文件,我们都要
            return true;
          }
          return false;
      ...
      if (dataFiles != null && dataFiles.length > 0) {//存在合适的文件名字
        for (File dataFile : dataFiles) {//循环每一个文件
  ...
  private boolean parseCommandLineOpts(String[] args) throws ParseException {
    ...
    if (commandLine.hasOption("eventValidator")) {//获取事件校验器的实现类
      try {
        ...
        EventValidator.Builder eventValidatorBuilder = eventValidatorClassName.newInstance();//创建实例对象
        ...
        eventValidator = eventValidatorBuilder.build();//创建校验器对象
      } catch (Exception e) {
